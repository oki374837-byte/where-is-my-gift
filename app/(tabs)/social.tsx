...
export default function SocialScreen() {
  ...
  const inviteCode = profile.data?.friendCode ?? "";
  ...
  const shareInvite = async () => {
    if (!inviteCode) {
      setNotice("سجّل الدخول أولًا لإنشاء كود دعوة خاص بك.");
      return;
    }
    await Share.share({
      title: "دعوة إلى أين هديتي؟",
      message: `انضم إليّ في لعبة أين هديتي؟ أضفني كصديق بهذا الكود: ${inviteCode}\n${inviteUrl}`,
      url: inviteUrl,
    });
  };
  ...
  const header = (
    <View>
      <Text style={[styles.eyebrow, { color: colors.primary }]}>WORLDQUEST / SOCIAL</Text>
      ...
      {!isAuthenticated ? (
        <View style={[styles.notice, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <Text style={[styles.noticeTitle, { color: colors.foreground }]}>سجّل الدخول لفتح الأصدقاء</Text>
          <Text style={[styles.noticeBody, { color: colors.muted }]}>تحتاج الصداقة والدردشة إلى حساب حتى لا تختلط الرسائل بين الأجهزة.</Text>
          <Pressable onPress={() => { void startOAuthLogin(); }} style={({ pressed }) => [styles.primaryButton, { backgroundColor: colors.primary, opacity: pressed ? 0.8 : 1 }]}>
