...
const ACHIEVEMENTS = [
  ...
  { id: "collector", title: "جامع الأسرار", detail: "اجمع 10 عناصر من رحلتك", target: 10, icon: "✦" },
  { id: "xp", title: "خريطة الخبرة", detail: "اجمع 1,000 نقطة خبرة", target: 1000, icon: "◎" },
  { id: "coins", title: "مؤن الرحلة", detail: "احتفظ بـ 250 عملة", target: 250, icon: "◈" },
  { id: "master", title: "مستكشف المدن", detail: "اكتشف 18 نقطة عبر المابات الثلاث", target: 18, icon: "⌖" },
];
...
export default function AchievementsScreen() {
  ...
  const values = { first: gameState.visitedCount, walker: gameState.distanceWalkedMeters, collector: collectedItems, xp: gameState.xp, coins: gameState.coins, master: gameState.visitedCount };
  const rows = ACHIEVEMENTS.map((item) => { const value = values[item.id as keyof typeof values]; const done = value >= item.target; const progress = item.id === "walker" ? `${Math.min(value / 1000, 5).toFixed(1)} / 5 كم` : `${Math.min(value, item.target)} / ${item.target}`; return { ...item, done, progress }; });
