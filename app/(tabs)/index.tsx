...
export default function HomeScreen() {
  ...
  const syncLabel = !isOnline
    ? `غير متصل · ${gameState.pendingActions.length} عمليات محفوظة محلياً`
    : socialFriends.isError
      ? "الخادم غير متاح · نستخدم آخر بيانات محفوظة"
  ...
  const travelToSelectedPoint = () => {
    ...
    const destination = { latitude: selectedPoint.latitude, longitude: selectedPoint.longitude };
    setPlayerLocation(destination);
    recordLocation(destination);
    setLocationState(`وصلت إلى ${selectedPoint.title} داخل عالم اللعبة`);
  };
  ...
  const dailyMissions = [
    { id: "discover", label: "اكتشف نقطتين من الماب", value: Math.min(activePoints.filter((point) => collected.includes(point.id)).length, 2), target: 2 },
    { id: "coins", label: "اجمع 50 عملة", value: Math.min(coins, 50), target: 50 },
    { id: "xp", label: "اجمع 100 XP", value: Math.min(xp, 100), target: 100 },
  ];
      ...
      <View style={styles.root}>
          ...
          <View style={[styles.headerGlass, { backgroundColor: "rgba(11, 43, 21, 0.88)", borderColor: "#22C55E" }]}>
            ...
            <View style={styles.headerActions}>
              ...
              <View style={[styles.levelBadge, { backgroundColor: "rgba(6, 35, 20, 0.9)", borderColor: "#22C55E" }]}>
                ...
                <Text style={[styles.levelValue, { color: "#FEF08A" }]}>{String(getCharacterLevel(xp)).padStart(2, "0")}</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        {selectedPoint && (
          <View style={[styles.sheet, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            ...
            {realityMode === "game-world" && distanceInMeters(playerLocation, selectedPoint) > INTERACTION_RADIUS_METERS && !collected.includes(selectedPoint.id) && (
              <Pressable onPress={travelToSelectedPoint} style={({ pressed }) => [styles.secondaryActionButton, { borderColor: selectedPoint.color, opacity: pressed ? 0.8 : 1 }]}>
                <Text style={[styles.secondaryActionButtonText, { color: selectedPoint.color }]}>انتقل إلى النقطة داخل الماب</Text>
              </Pressable>
            )}
...
function GameMapCanvas({
  ...
  return (
    <View style={styles.detailMap}>
      ...
      {map.zones.map((zone) => (
        <View key={zone.id} style={[styles.zoneLabel, { left: `${zone.x}%`, top: `${zone.y}%`, borderColor: map.accent }]}>
          <Text style={[styles.zoneLabelText, { color: map.accent }]}>{zone.label}</Text>
        </View>
      ))}
...
const styles = StyleSheet.create({
  ...
  zoneLabel: { position: "absolute", minWidth: 72, paddingHorizontal: 7, paddingVertical: 5, borderWidth: 1, borderRadius: 9, backgroundColor: "rgba(3, 15, 22, 0.58)", alignItems: "center", zIndex: 2 },
  zoneLabelText: { fontSize: 9, fontWeight: "900", textAlign: "center" },
  ...
  dailyCard: { borderWidth: 1, borderRadius: 16, padding: 12, marginBottom: 10 },
  ...
  dailyHint: { fontSize: 10 },
  dailyRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", paddingVertical: 5 },
  dailyLabel: { fontSize: 11 },
  dailyProgress: { fontSize: 11, fontWeight: "900" },
  ...
  secondaryActionButton: { borderWidth: 1.5, borderRadius: 14, paddingVertical: 11, alignItems: "center", marginTop: 10 },
  secondaryActionButtonText: { fontSize: 13, fontWeight: "900" },
});
