...
export default function ProfileScreen() {
  ...
  useEffect(() => {
    let active = true;
    void loadSelectedCharacter().then((character) => {
    ...
    <ScreenContainer className="p-5">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 28 }}>
        ...
        <View style={[styles.settings, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          <View style={styles.settingRow}><View><Text style={[styles.settingTitle, { color: colors.foreground }]}>الاهتزاز</Text><Text style={[styles.settingHint, { color: colors.muted }]}>تغذية لمسية عند جمع المكافآت</Text></View><Switch value trackColor={{ false: colors.border, true: colors.primary }} thumbColor="#F4F8FC" /></View>
          <View style={[styles.divider, { backgroundColor: colors.border }]} />
          <View style={styles.settingRow}><View><Text style={[styles.settingTitle, { color: colors.foreground }]}>تنبيه الاستكشاف اليومي</Text><Text style={[styles.settingHint, { color: colors.muted }]}>تذكير محلي في السادسة مساءً</Text></View><Switch value={remindersEnabled} onValueChange={async (enabled) => { setRemindersEnabled(enabled); if (enabled) { const granted = await enableExplorationReminder(); if (!granted) setRemindersEnabled(false); } else await disableExplorationReminder(); }} trackColor={{ false: colors.border, true: colors.primary }} thumbColor="#F4F8FC" /></View>
          <View style={[styles.divider, { backgroundColor: colors.border }]} />
