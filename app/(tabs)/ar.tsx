...
import { loadRealityMode, loadSelectedMap } from "@/lib/map-storage";
...
export default function ARScreen() {
  ...
  useEffect(() => {
    void Promise.all([loadSelectedCharacter(), loadSelectedMap(), loadRealityMode()]).then(([character, mapId, mode]) => {
  ...
  useEffect(() => {
    ...
    start().catch(() => setLocationReady(false));
    return () => {
      mounted = false;
  ...
  const questMarkers = useMemo<ArMarker[]>(() => getMapPoints(selectedMap)
    ...
    .filter((marker) => marker.distance <= 500)
    ...
    <View style={styles.full}>
      ...
      <View style={styles.arMiniMap}>
        <WorldMap
          ...
          livePlayers={[]}
        />
        <View style={styles.arMiniMapLabel}><Text style={styles.arMiniMapLabelText}>خريطة حقيقية · {currentMap.city}</Text></View>
      </View>
